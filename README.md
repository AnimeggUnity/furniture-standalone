...
- ✅ **得標者分組智慧排序**：得標者群組依三層優先級排列。
  ...
  - 第三層：全數取完 → 排最後。
  - 同層內維持姓名字母排序。
- ✅ **操作按鈕防呆**：已得標商品顯示灰色唯讀「編輯」按鈕（可查閱截標前內容，所有欄位不可修改）；已結標/已得標商品隱藏「即時結標」，按鈕位置保留避免版面跳動。
- ✅ **年度統計月份明細**：季度表格各格新增月份子表，筆數／得標／付款／取貨／收入分欄顯示並靠右對齊。Q4 跨年月份（1–2月）正確對應次一日曆年資料。
- ✅ **追蹤數排序**：新增「追蹤數排序」按鈕，過濾未得標商品並依追蹤數降冪排列；與「得標者分組」互斥，切換時自動重置對方狀態。
